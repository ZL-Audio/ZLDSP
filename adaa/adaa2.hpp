...
namespace zldsp::adaa {
    ...
    public:
        ...
        void resetADAA() {
            // re-calculate states
            g2_x1_ = g2(x1_);
            const auto x1_x0_delta = x1_ - x0_;
            FloatType d1 = std::abs(x1_x0_delta) > kEps
                               ? (g2_x1_ - g2(x0_)) / (x1_x0_delta) // x1 - x0 normal condition
                               : g1(FloatType(0.5) * (x1_ + x0_)); // x1 - x0 ill condition
        }
        ...
        FloatType processADAA(FloatType x2) {
            ...
            if (std::abs(x2_x0_delta) > kEps) {
                // x2 - x0 normal condition
